                    ...
                    return(
                     <li key={item._id}>
                        ...
                        <div className="collapsible-body">
                            <h5 style={{marginLeft: '3%'}}>Total  Rs. {item.total}</h5>
                            {
                                item.products.map(pitem=>{
                                    return <img src={pitem.product.mediaUrl} style={{maxWidth: '20%'}} />
                                })
                            }
                            {
                                item.products.map(pitem=>{
                                  return <h6 key={pitem._id}> Name: {pitem.product.name} X Quantity: {pitem.quantity}</h6>  
                                })
                            }
                        </div>
                    </li>   
